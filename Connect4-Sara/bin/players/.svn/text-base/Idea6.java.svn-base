 ...
 * The difference between this player and Idea4 and Idea5 is that
 * it only detects the winning moves of the opponent and dangerous
 * moves with the 50% of possibility.
 */
public class Idea6 extends AbstractPlayer {
...
//			System.out.println("lastMove p1:" + "("+last_move[0]+ ","+ last_move[1]+")" + "Running Total:"+ P1RunningTotal);
		...
		System.out.println("PID = " + playerID + ": " + "min=" + min + " " + "max=" + max);

		/* 
		 * checks if the move is in the allowed range
		 * if it is in the allowed range, adds it to the possibleMoves list
		 */
...
//			System.out.println("First AI RT: "+ firstAIRT);
		...
		for (Move myMove : game.getAvailableMoves()) {		
			...
			/* only detects the winning move of the human player with 50% of the chance*/
			if(rng.nextBoolean()){
				...
				for (Move hisMove : game.getAvailableMoves()) {
					...
					if (game.isWon()) {
						/* 
						 * if both AI and human moves are the same it means that
						 * the AI players move allowed the human player to win
						 * therefore we add them to the dangerous moves, otherwise
						 * we add them to the blocking moves to prevent human player
						 * from winning. 
						 */
						if(hisMove != myMove) {
		 ...
		 * cannot win in the next turn, therefore all 
		 *	the available moves are safe moves
		 */
		...
		/* 
		 * AI lost the game, because all the available moves are dangerous
		 * so it must choose one of the dangerous moves to complete the game.
		 */
...
//			System.out.println("AI RT updated: "+ P1RunningTotal);
		} else {
			P2RunningTotal += evalPosition(game, x, y, playerID);
//			System.out.println("AI RT updated: "+ P2RunningTotal);
		}
